...
`gsuite-mcp-gateway` is a stateless Cloudflare Worker that exposes a remote MCP server over Streamable HTTP and acts as an OAuth authorization server for ChatGPT while brokering Google OAuth to Calendar, Drive, and Gmail.
...
- OAuth Authorization Code + PKCE S256 for ChatGPT, plus Google OAuth web server flow for Google Calendar, Drive, and Gmail.
- Calendar tools: list calendars/events, get event, freebusy, create/update/delete events, attendees, sendUpdates, Google Meet.
- Drive tools: list/search files, get metadata, upload small files, download/export small files, delete files.
